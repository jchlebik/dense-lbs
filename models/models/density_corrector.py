...
class Density_Corrector(nn.Module):
    stages: int = 4
    channels: int = 8
    ...
    def get_keys():
        ...
        density_key = keys_mapping[km.get_density_key()]
        pml_key = keys_mapping[km.get_pml_key()]
        source_key = keys_mapping[km.get_source_key()]
        sos_field_key = keys_mapping[km.get_sos_field_key()]
        return [sos_key, density_key, pml_key, source_key, sos_field_key]
    ...
    def __call__(self, **kwargs) -> jnp.ndarray:
        ...
        # Pad input
        sos_field_r = jnp.real(sos_field)
        if self.padding > 0:
            src = jnp.pad(
                src, 
        ...
        x = jnp.concatenate([src, sos_field_r, density_context], axis=-1)
        skips = []

        for i in range(self.stages):
            skip, x = self.downsample_block(
                x,
    ...
    def downsample_block(self, x, channels, kernel_size, strides, padding = 'SAME'):
        skip = self.double_conv(
            x, 
        ...
        pool_down = nn.avg_pool(skip, window_shape=(2, 2), strides=(2, 2))
        return skip, pool_down
    
    def upsample_block(self, x, skip, channels, kernel_size, strides, padding = 'SAME'):
    ...
    def bottleneck_block(self, x, channels, kernel_size, strides, padding='SAME'):
        x = self.double_conv(
            x, 
            middle_channels=channels * 2, 
            out_channels=channels, 
    ...
    def double_conv(self, x, middle_channels, out_channels, kernel_size=(3, 3), strides=1, padding='SAME'):
